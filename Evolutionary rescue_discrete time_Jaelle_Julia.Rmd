...
-   $N__end_wd$ size of the mutant (wd) population
-   $N__end_wb$ size of the mutant (wb) population
-   $N__end_sd$ size of the mutant (sd) population
-   $N__end_sb$ size of the mutant (sb) population
...
-   `s_wm`selective strength of the strong mutation (should be \>decay_rate to allow for evolutionary rescue)
    ...
    for(sval_wm in s_values_wm){
      for(sval_sm in s_values_sm){
        ...
        repeat {
          ...
          rescued <- ifelse(total_end > 0,1,0)
          rescued_count <- rescue_count + rescued
          # save rescue outcome (rescue or extinction)
          rescue_results <- rbind(rescue_results,
                                  data.frame(s_wm = sval_wm,
                                             s_sm = sval_sm,
                                             replicate = i-1,
                                             rescued = rescued))
...
rescue_summary <- rescue_results %>%
  group_by(s_wm, s_sm) %>%
  summarise(rescue_prob = mean(rescued), .groups = "drop")


print(rescue_summary)
library(ggplot2)
# plotting rescue probability with a heatmap
...
ggplot(data_table_wd, aes(x = factor(V1), y = V2, color = factor(V1))) +
  ...
  labs(
    ...
    title = "Weak deletrious-Means of frequencies of the different selection coefficients"
  ) +
  theme_minimal()
...
print(paste("The rescue probability is =",rescue_prob))
