      ...
      for(sval_sm in s_values_sm){
        ...
        repeat {
          ...
          one_run <- simulate_pop(init_wd, init_wb, init_sd, init_sb,
                                  ...
                                  sval_wm, sval_sm, m_rate_wm=0.001,m_rate_sm=0.001, max_gen)
          ...
          data_table <- rbind(data_table,c(m_rate_wm = 0.001,m_rate_sm = 0.001,sval_wm,sval_sm,N_end_wd,        N_end_wb, N_end_sd, N_end_sb,freq_wd, freq_wb, freq_sd,freq_sb)) # note that we add the                varying parameters (mutation rate and selection strength for each version of weak            and strong mutation) to the table too
...
rescue_long <- rescue_summary %>%
  ...
  mutate(
    ...
    # Definiere, welche s-Achse für welchen Mutant gilt:
    s_value = case_when(
...
# optional: Mutantennamen schöner machen
rescue_long$mutant <- gsub("rescue_prob_", "", rescue_long$mutant)

ggplot(rescue_long, aes(x = s_value, y = rescue_prob, color = mutant, group = mutant)) +
  geom_point(size = 2) +
  ...
  labs(
    title = "Rescue Probability per Mutant Type",
...
# plotting rescue probability and s values with a heatmap
ggplot(rescue_summary, aes(x = factor(s_wm), y = factor(s_sm), fill = rescue_prob)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "white", high = "steelblue", limits = c(0,1)) +
  labs(
    title = "Rescue Probability across parameter combinations",
    x = "s_wm (Selection coefficient, weak mutation)",
    y = "s_sm (Selection coefficient, strong mutation)",
    fill = "Rescue probability"
  ) +
  theme_minimal()
