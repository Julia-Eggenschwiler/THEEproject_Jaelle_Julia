...
simulate_one_gen <- function(N_wd, N_wb, N_sd, N_sb, decay_rate_wm, decay_rate_sm, s_wm, s_sm, m_rate_wm, m_rate_sm) {
  ...
  N_wd_new <- offsp_wd - mut_wd_to_wb+ mut_wb_to_wd
  N_wb_new <-offsp_wb - mut_wb_to_wd+ mut_wd_to_wb
  N_sd_new <- offsp_sd - mut_sd_to_sb+ mut_sb_to_sd
  N_sb_new <-offsp_sb - mut_sb_to_sd+ mut_sd_to_sb
...
simulate_pop <- function(init_wd, init_wb,  init_sd, init_sb, decay_rate_wm, decay_rate_sm, s_wm, s_sm, m_rate_wm, m_rate_sm, max_gen) {
  ...
  for (i in 1:(max_gen + 1)) {
    ...
    if ((sum(pop_new) >= 4000 * sum(c(init_wd, init_wb, init_sd, init_sb))) || sum(pop_new) == 0)
      break
...
output <- simulate_pop(init_wd=100, init_wb=0,  init_sd=100, init_sb=0, decay_rate_wm=0.003, decay_rate_sm=0.009, s_wm=0.2, s_sm=0.4, m_rate_wm=0.001, m_rate_sm=0.001,max_gen)
      ...
      for(sval_sm in s_values_sm){
        ...
        repeat {
          ...
          one_run <- simulate_pop(init_wd, init_wb, init_sd, init_sb,
                                  ...
                                  sval_wm, sval_sm, m_rate_wm=0.2,m_rate_sm=0.2, max_gen)
