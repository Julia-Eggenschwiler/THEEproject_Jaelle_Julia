...
print(simulate_one_gen(N_wd=30, N_wb=0, N_sd=30, N_sb=0, decay_rate_wm=0.045, decay_rate_sm=0.09, s_wm=0.1, s_sm=0.2, m_rate_wm=0.05, m_rate_sm=0.07))
```
...
output <- simulate_pop(init_wd=30, init_wb=0,  init_sd=30, init_sb=0, decay_rate_wm=0.025, decay_rate_sm=0.05, s_wm=0.1, s_sm=0.2, m_rate_wm=0.05, m_rate_sm=0.07,max_gen)
...
for (rep in 1:no_replicates) {
  ...
  outcome <-
    if (weak_alive & !strong_alive) "Weak wins" else
      if (strong_alive & !weak_alive) "Strong wins" else
        "Total extinction"
...
competition_results$outcome <- factor(
  ...
  levels = c("Weak wins", "Strong wins", "Total extinction")
)
