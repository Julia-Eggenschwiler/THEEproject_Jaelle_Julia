...
simulate_one_gen <- function(N_wd, N_wb, N_sd, N_sb, decay_rate_wm, decay_rate_sm, s_wm, s_sm, m_rate_wm, m_rate_sm) {
  ...
  # with the return function, the new values for population sizes are stored
   ...
   # Rückgabe: neue Populationsgrößen UND Mutationszahlen
  return(list(
    N_wd_new = N_wd_new,
    N_wb_new = N_wb_new,
    N_sd_new = N_sd_new,
...
simulate_pop <- function(init_wd, init_wb,  init_sd, init_sb, decay_rate_wm, decay_rate_sm, s_wm, s_sm, m_rate_wm, m_rate_sm, max_gen) {
  ...
  pop_new <- c(init_wd, init_wb, init_sd, init_sb)
   ...
   # Neue Zähler für Mutationswechsel
  count_wb_to_wd <- 0
  count_wd_to_wb <- 0
  count_sb_to_sd <- 0
  count_sd_to_sb <- 0
  ...
  # Rückgabe wie bisher + neue Statistik
  return(list(
...
x_range <- 0:(length(output$population[,1])-1)
...
# Liste vorbereiten, um die Mutationswechsel für alle Replikate zu speichern
switch_stats_all <- rbind(switch_stats_all,
                          data.frame(
                            s_wm = sval_wm,
                            s_sm = sval_sm,
                            replicate = i-1,
                            wb_to_wd = 100 * switch_stats$wb_to_wd,
                            wd_to_wb = 100 * switch_stats$wd_to_wb,
                            sb_to_sd = 100 * switch_stats$sb_to_sd,
                            sd_to_sb = 100 * switch_stats$sd_to_sb
                          ))
...
#for(mval_wm in m_values_wm){
 ...
 if (!is.null(one_run$switch_stats) && nrow(one_run$switch_stats) > 0) {
  ...
  #}
#}
# Population über Generationen
print(one_run$population)

# Mutationswechsel in Prozent
